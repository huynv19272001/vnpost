...
@Slf4j
@RestController
@RequestMapping("api/cchqvn/")
public class ChiCucHaiQuanVnController {

    @Autowired
    ChiCucHaiQuanVnService chiCucHaiQuanVnService;

    @GetMapping("list")
    public ResponseEntity<?> getList() {
        try {
            List<ChiCucHaiQuanVn> chiCucHaiQuanVns =  chiCucHaiQuanVnService.getList();
            return new ResponseEntity<>(new ResponseWrapper(chiCucHaiQuanVns), HttpStatus.OK );
